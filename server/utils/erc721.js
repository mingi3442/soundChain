...
module.exports = {
  mintNFT: async (imgUri, createrAddress, createrPK, ownerId, ownerName, title, desc, owner_id) => {
  ...
  sendNFT: async (ownerPK, ownerAddress, nftTokenId, nftId, otherAddress, otherUserId, ownerUserId) => {
    const server = process.env.TOKEN_ADDRESS;
    var erc721contract = new Contract(erc721contractABI, process.env.ERC721ADDR);
    const nftData = erc721contract.methods.transferFrom(ownerAddress, otherAddress, parseInt(nftTokenId)).encodeABI();
    const rawTransaction = {
      from: server,
      to: process.env.ERC721ADDR,
      gas: 5000000,
    ...
    web3.eth.accounts.signTransaction(rawTransaction, ownerPK).then((signTx) => {
      web3.eth.sendSignedTransaction(signTx.rawTransaction, async (err, req) => {
        if (err) {
          ...
          const nft = Nft.updateOne({ _id: nftId }, { $set: { ownerId: otherUserId, price: 0, buyable: false, prevOwner: ownerUserId } });
          return nft;
        }
      });
    });
