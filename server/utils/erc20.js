...
module.exports = {
          ...
          if (!err) {
            await contract.methods
              ...
              .then((balance) => {
                ...
                const user = User.updateOne({ _id: userId }, { erc20: balance });
                return user;
  ...
  sendToken: async (userId, otherUserId, value) => {
    ...
    const txData = contract.methods.transfer(otherUser.address, value).encodeABI();
    const rawTransaction = {
      to: process.env.ERC20ADDR,
      gas: 100000,
      data: txData,
          ...
          if (!err) {
            await contract.methods
              ...
              .then(async (balance) => {
                ...
                await User.updateOne({ _id: otherUserId }, { erc20: balance });
                await User.updateOne({ _id: userId }, { $inc: { erc20: -value } });
                return true;
              });
          } else {
            console.log("실패", err);
